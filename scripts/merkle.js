...
function main() {
  ...
  const filename = path.join(__dirname, "genfile/first.csv");

  // what file should we write the merkle proofs too?
  const output_file = path.join(__dirname, "genfile/small.json");

  // file that has the user claim list
  const userclaimFile = path.join(__dirname, "genfile/small2.json");

  // contract of items being sent out
  const airdropContract = "0xD82faA5Fa56129569D9a6A547C09ff82B532602b";
